  ...
  <div class="space-y-6">
    <div class="card">
      <div class="flex items-center justify-between">
        <div>
          <h2 class="text-base font-semibold">新增系统资产</h2>
          <p class="text-sm text-muted-foreground">填写系统基础信息与自定义字段。</p>
        </div>
      ...
      <div v-for="category in categories" :key="category.id" class="space-y-3" v-show="isCategoryVisible(category)">
        ...
        <div class="form-grid-2 xl:grid-cols-3 form-grid-divider-md-2-xl-3">
          ...
          >
            ...
            <div
              v-else-if="field.field_type === 'markdown'"
              class="space-y-2"
              :class="inputErrorClass(fieldErrorKey(field))"
              :data-error-key="fieldErrorKey(field)"
            >
            ...
            <div
              v-else-if="field.field_type === 'topology'"
              class="space-y-2"
              :class="inputErrorClass(fieldErrorKey(field))"
              :data-error-key="fieldErrorKey(field)"
            >
...
const validateRequired = () => {
  ...
  for (const field of allFields) {
    ...
    // app_code is auto-generated by backend; do not block submit on empty value.
    if (isBaseField(field) && field.field_key === "app_code") continue;
