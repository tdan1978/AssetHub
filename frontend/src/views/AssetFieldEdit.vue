...
const categoryScope = ref("global");
...
const filteredDictTypes = computed(() => {
  ...
  return dictTypes.value.filter((item) => item.scope === "global" || item.scope === scope);
});

const load = async () => {
  const { data } = await api.get(`/categories/${categoryId}/fields`);
  existingFields.value = data.filter((item) => item.id !== fieldId);
  const item = data.find((f) => f.id === fieldId);
  if (!item) return;
  form.value = { ...item };
  if (form.value.field_type === "combo_select") {
    form.value.field_type = "single_select";
  }
  options.value =
    Array.isArray(item.options) && item.options.every((opt) => typeof opt === "string")
      ? item.options.join(",")
      : "";
  form.value.reminder_enabled = Boolean(item.reminder_enabled);
  form.value.reminder_days = item.reminder_days ?? null;
  form.value.data_source = item.data_source || "static";
  form.value.searchable = item.field_type === "combo_select" ? true : Boolean(item.searchable);
  form.value.multi_select_mode = item.multi_select_mode || "checkbox";
  form.value.multi_select_mode = item.multi_select_mode || "checkbox";
  if (item.field_type === "compound") {
    compoundFields.value = Array.isArray(item.options) && item.options.length
  ...
  if (item.data_source && item.data_source.startsWith("dict:")) {
    form.value.data_source = "dict";
    dictTypeCode.value = item.data_source.replace("dict:", "");
  }
  if (!canRepeat.value) {
  ...
  const currentRule = Array.isArray(item.visibility_rules) ? item.visibility_rules[0] : null;
  if (currentRule) {
...
const loadCategoryScope = async () => {
  try {
    const { data } = await api.get("/categories");
    const current = Array.isArray(data) ? data.find((item) => item.id === categoryId) : null;
    categoryScope.value = current?.usage_scope || "global";
  } catch {
...
watch(
  ...
  () => {
    if (form.value.data_source === "dict") {
      const exists = filteredDictTypes.value.some((item) => item.code === dictTypeCode.value);
      if (!exists) {
        dictTypeCode.value = "";
      }
    }
