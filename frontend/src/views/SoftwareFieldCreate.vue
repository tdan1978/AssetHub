  ...
  <div class="space-y-6">
    <div class="card">
      <div class="flex items-center justify-between">
        <div>
          <h2 class="text-base font-semibold">新增软件字段</h2>
          <p class="text-sm text-muted-foreground">为软件资产添加自定义字段。</p>
        </div>
...
const save = async () => {
  ...
  const compoundPayload =
    form.value.field_type === "compound"
      ...
      : null;
  await api.post(`/software-field-categories/${categoryId}/fields`, {
    ...form.value,
...
const loadFields = async () => {
  ...
  existingFields.value = data.flatMap((item) => item.fields || []);
};
