  ...
  <div class="space-y-6">
    <div class="card">
      <div class="flex items-center justify-between">
        <div>
          <h2 class="text-base font-semibold">新增部门</h2>
          <p class="text-sm text-muted-foreground">创建部门并支持设置上级。</p>
        </div>
        ...
        <div class="form-field">
          ...
          <Select v-model="form.parent_id">
            ...
            <SelectContent>
              ...
              <SelectItem v-for="item in departmentOptions" :key="item.id" :value="String(item.id)">
                {{ item.name }}
              </SelectItem>
            </SelectContent>
          </Select>
...
const loadOptions = async () => {
  ...
  const preset = route.query.parent_id ? String(route.query.parent_id) : noneValue;
  form.value.parent_id = preset;
};
