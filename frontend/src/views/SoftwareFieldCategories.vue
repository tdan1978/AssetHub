  ...
  <div class="space-y-6">
    <div class="card">
      <div class="flex items-center justify-between">
        <div>
          <h2 class="text-base font-semibold">软件字段分类</h2>
          <p class="text-sm text-muted-foreground">管理软件资产字段分类。</p>
        </div>
        <RouterLink to="/software-field-categories/new">
          ...
          <tr v-for="item in categories" :key="item.id" class="border-b">
            ...
            <td class="px-4 py-2">
              <div class="flex gap-2">
                <RouterLink :to="`/software-field-categories/${item.id}/fields`">
                  <Button size="sm" variant="default">字段</Button>
                </RouterLink>
                <RouterLink :to="`/software-field-categories/${item.id}/edit`">
...
const confirmDelete = async () => {
  ...
  await api.delete(`/software-field-categories/${pendingId.value}`);
