  ...
  <div class="space-y-6">
    <div class="card">
      <div class="flex items-center justify-between">
        <div>
          <h2 class="text-base font-semibold">编辑软件字段</h2>
          <p class="text-sm text-muted-foreground">更新软件资产字段。</p>
        </div>
        <RouterLink :to="`/software-field-categories/${categoryId}/fields`">
...
const load = async () => {
  const { data } = await api.get(`/software-field-categories/${categoryId}/fields`);
  const { data: tree } = await api.get("/software-field-categories/tree");
...
const save = async () => {
  ...
  const compoundPayload =
    form.value.field_type === "compound"
      ...
      : null;
  await api.put(`/software-field-categories/fields/${fieldId}`, {
    ...form.value,
  ...
  router.push(`/software-field-categories/${categoryId}/fields`);
};
